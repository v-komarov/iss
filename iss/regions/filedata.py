...
def get_orders_region(request):
    ...
    if request.GET.has_key("action") and request.GET["action"] == "getorders":
        ...
        response_data = u"№п/п;Артикул/марка;Наименование товара;Ед.изм.;Кол-во;Начальная цена за единицу товара, в руб. без НДС с учетов всех расходов;Начальная цена договора, в руб. без НДС с учетов всех расходов;Коментарий;\n"

        for row in orders.objects.filter(region=region).order_by('order'):
            price = ("%s" % row.price).replace(".", ",")
            rowsum = ("%s" % row.rowsum).replace(".", ",")
            response_data += u"{order};{model};{name};{ed};{count};{price};{rowsum};\n".format(order=row.order, model=row.model, name=row.name, ed=row.ed, count=row.count, price=price, rowsum=rowsum)
