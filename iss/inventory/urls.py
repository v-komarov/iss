...
from iss.inventory.views import DeviceSchemeList,NetElementsList,NetElement,DevicesList,Device,DevicesAuditPorts, DevicesIp
...
urlpatterns = [
    ...
    url(r'netelementdata/(?P<pk>\d+)/$', NetElement.as_view()),
    url(r'devicedata/(?P<pk>\d+)/$', Device.as_view()),
    url(r'jsondata/$', get_json),
    url(r'scheme_device/$', get_device_scheme),
    url(r'audit_ports/$', get_audit_ports),
    url(r'devicesip/$', DevicesIp.as_view()),
]
